...
if __name__ == '__main__':
    ...
    # build global model(from class Cifar100WEIT)
    net_glob = Cifar100WEIT([3,32,32])
    # Do the global training! training mode
    net_glob.train()
    if args.load_fed != 'n':
    ...
    # generate list of local models for each user save every value of the model in the dict for every user
    net_local_list = []
    w_locals = {}
    ...
    for iter in range(args.epochs):
        ...
        if iter % args.round == args.round-1:
            ...
            model_save_path = './save/WEIT/' + args.dataset + '_' + str(args.num_users) + '_' + str(
    ...
    print(times)
    print(accs)
    base_dir ='./save/WEIT/' + args.dataset + '_' + str(args.num_users) + '_' + str(
